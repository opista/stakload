import type { GameDto, InvolvedCompanyDto, ReferenceItemDto } from "../models/game.dto";
...
export const mapAggregatedGameToDto = (rawGame: RawAggregatedGameDto | string): GameDto => {
  ...
  } = parsedGame as RawAggregatedGameDto & Partial<Pick<GameDto, "developers" | "publishers">>;
  ...
  return {
    ...
    developers: involvedCompanies.length ? deriveCompanyReferences(involvedCompanies, "developer") : (existingDevelopers ?? []),
    ...
    publishers: involvedCompanies.length ? deriveCompanyReferences(involvedCompanies, "publisher") : (existingPublishers ?? []),
