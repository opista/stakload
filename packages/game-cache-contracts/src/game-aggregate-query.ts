...
export interface GameBuildQueryRow {
  game: RawAggregatedGameDto | string | null;
}

export const GAME_AGGREGATE_QUERY_BY_IDS = `
  ...
  FROM games g
  WHERE g."igdbId" = ANY($1::int[])
`;

export const mapGameBuildRows = (rows: GameBuildQueryRow[]): GameDto[] =>
  rows.flatMap((row) => (row.game === null ? [] : [mapAggregatedGameToDto(row.game)]));
