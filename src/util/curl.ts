...
const getCurlPath = () => {
  ...
  if (window.NL_OS === "Windows") {
    return [window.NL_CWD, extensionPath, `windows-${window.NL_ARCH}`, "bin"].join("/");
  }

  return [window.NL_PATH, extensionPath, `mac-${window.NL_ARCH}`, "bin"].join("/");
};

export const curl = async <T>(url: string, options?: RequestInit): Promise<T> => {
  const command = fetchToCurl(url, options);

  const result = await executeCommand(`${getCurlPath()}/${command} -k`);
