...
export const QuickLaunchItem = ({ editMode, game }: QuickLaunchItemProps) => {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <div
        className={cn(
          "group flex cursor-pointer items-center gap-2 rounded-lg p-2 pr-3 transition-colors text-slate-400",
          !editMode && "hover:bg-primary/10 hover:text-primary",
        )}
        onClick={onClick}
      >
        ...
        <div className="relative flex w-full min-w-0 flex-col text-xs font-bold uppercase">
          <span className="block w-full truncate tracking-wider transition-transform duration-300 ease-in-out group-hover:-translate-y-1/2">
            {game.name}
          </span>
